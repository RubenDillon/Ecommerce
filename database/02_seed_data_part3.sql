...
SELECT 
    ...
    MIN(p.Valor_venta) AS PrecioMinimo,
    MAX(p.Valor_venta) AS PrecioMaximo
FROM 
    Clases c
...
SELECT COUNT(*) AS TotalProductos, SUM(Stock) AS StockTotal FROM Productos;

SELECT 'Valor total del inventario:' AS Info;
SELECT 
    SUM(Valor_costo * Stock) AS ValorCostoTotal,
    SUM(Valor_venta * Stock) AS ValorVentaTotal,
    SUM((Valor_venta - Valor_costo) * Stock) AS MarcaTotal
FROM Productos;
